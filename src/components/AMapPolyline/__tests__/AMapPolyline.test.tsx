...
describe('AMapPolyline Component', () => {
  ...
  test('renders without crashing', () => {
    expect(() => {
      render(<AMapPolyline path={mockPath} />);
    }).not.toThrowError();
    expect(useAMapPluginInstance).toHaveBeenCalledWith('Polyline', expect.any(Function));
  });
  ...
  test('support ref to instance', () => {
    ...
    const { rerender } = render(<AMapPolyline path={mockPath} ref={$ref} />);
    expect($ref.current).toBe(null);
    rerender(<AMapPolyline path={mockPath} ref={$ref} />);
    expect($ref.current).toBe(mockInstance);
  });

  test('set ext-data', () => {
    const extData = { id: 1 };
    render(<AMapPolyline path={mockPath} extData={extData} />);

    expect(mockInstance.setExtData).toHaveBeenCalledWith(extData);
  });

  test('set to invisible', () => {
    const { rerender } = render(<AMapPolyline path={mockPath} />);

    expect(mockInstance.show).toBeCalled();

    rerender(<AMapPolyline path={mockPath} visible={false} />);
  ...
  test('sets options: custom style', () => {
    render(
      <AMapPolyline
        ...
        dirColor="#000"
        lineCap="round"
        dirImg="https://localhost:3000/dir.png"
        isOutline
        outlineColor="yellow"
        geodesic
      />,
